...
with st.sidebar:
    ...
    if t_file and c_file:
        if t_file.name != st.session_state['last_t_name'] or c_file.name != st.session_state['last_c_name']:
            ...
            # FIX: icon deve essere un'emoji valida
    ...
    with tabs[0]:
        if st.session_state['p1_data']:
            ...
            with c1:
                ...
                d['imagen_prompt'] = st.text_area("Prompt", d.get('imagen_prompt'), height=100, key="p1_p")
    ...
    with tabs[1]:
        if st.session_state['p2_data']:
            ...
            with c3:
                ...
                d2['imagen_prompt'] = st.text_area("Prompt Sfondo", d2.get('imagen_prompt'), height=100, key="p2_p")
                ...
                if st.session_state['p2_img']:
                    ...
                    if st.button("💾 SALVA P2", key="p2_sv"):
                        ...
                        if len(prs.slides) > 1:
                            ...
                            out = io.BytesIO(); prs.save(out); out.seek(0); st.session_state['ppt_buffer'] = out
                            st.success("Salvato!")
                        else: st.error("Manca Pagina 2 nel template")
        else: st.info("In attesa di analisi...")

    st.divider()
    st.download_button("SCARICA PPT FINALE", st.session_state['ppt_buffer'], "AI_PPT.pptx", type="primary", use_container_width=True)
else:
